...
class Tile(pygame.sprite.Sprite):
    def __init__(self, filename, x, y):
        ...
        self.original_image = pygame.image.load('images/villagers/' + filename)

        self.back_image = pygame.image.load('images/villagers/' + filename)
        pygame.draw.rect(self.back_image, WHITE, self.back_image.get_rect(), border_radius=15)
...
class Game():
    def __init__(self):
        ...
        # villagers
        self.all_villagers = [f for f in os.listdir('images/villagers') if os.path.join('images/villagers', f)]
        ...
        self.stop = pygame.image.load('images/pause.png').convert_alpha()
        self.video_toggle = self.play
        self.video_toggle_rect = self.video_toggle.get_rect(topright = (WINDOW_WIDTH - 80, 10))
        self.get_video()

        # initialize music
        self.is_music_playing = True
        self.sound_on = pygame.image.load('images/volume.png').convert_alpha()
    ...
    def generate_level(self, level):
        self.villagers = self.select_random_villagers(self.level)
        self.level_complete = False
        self.rows = self.level + 1
        self.cols = 4
        self.generate_tileset(self.villagers)

    def generate_tileset(self, villagers):
        ...
        for i in range(len(villagers)):
            ...
            tile = Tile(villagers[i], x, y)
            self.tiles_group.add(tile)


    def select_random_villagers(self, level):
        villagers = random.sample(self.all_villagers, (self.level + self.level + 2))
        villagers_copy = villagers.copy()
        villagers.extend(villagers_copy)
    ...
    def draw(self):
        ...
        # font
        title_font = pygame.font.Font('font/MilkyNice.ttf', 44)
        content_font = pygame.font.Font('font/MilkyNice.ttf', 24)

        # text
        title_text = title_font.render('XOLO-TILES', True, WHITE)
        ...
        info_text = content_font.render('Match 2 XOLO Villagers', True, WHITE)
        ...
        if not self.level == 5:
            next_text = content_font.render('Level complete! Press Space to get to the next level!', True, WHITE)
        else:
            next_text = content_font.render('AMAZING! You Won XOLO-TILES. Press Space to play again!', True, WHITE)
    ...
    def get_video(self):
        self.cap = cv2.VideoCapture('video/galaxy.mp4')
        self.success, self.img = self.cap.read()
